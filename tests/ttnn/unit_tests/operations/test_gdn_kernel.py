...
def test_gdn_kernel_correctness(mesh_device, reset_seeds, num_heads):
    ...
    query = torch.randn(batch_size, SEQ_LEN, num_heads, Dk, dtype=torch.float32)
    key = torch.randn(batch_size, SEQ_LEN, num_heads, Dk, dtype=torch.float32)
    value = torch.randn(batch_size, SEQ_LEN, num_heads, Dv, dtype=torch.float32)
    g = torch.randn(batch_size, SEQ_LEN, num_heads, dtype=torch.float32) * 0.5 - 1.0
    beta = torch.randn(batch_size, SEQ_LEN, num_heads, dtype=torch.float32)
