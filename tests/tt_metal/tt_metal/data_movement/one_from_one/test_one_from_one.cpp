...
bool run_dm(const shared_ptr<distributed::MeshDevice>& mesh_device, const OneFromOneConfig& test_config) {
    ...
    std::unordered_map<std::string, uint32_t> requestor_compile_args = {
    ...
    if (MetalContext::instance().get_cluster().arch() == ARCH::QUASAR) {
        requestor_kernel = experimental::quasar::CreateKernel(
            ...
            experimental::quasar::QuasarDataMovementConfig{
                .num_threads_per_cluster = 1, .named_compile_args = requestor_compile_args});
    } else {
        requestor_kernel = CreateKernel(
            ...
            DataMovementConfig{
                ...
                .named_compile_args = requestor_compile_args});
    }
...
TEST_F(QuasarMeshDeviceSingleCardFixture, TensixDataMovementOneFromOnePacketSizes) {
    uint32_t test_id = 916;
...
TEST_F(QuasarMeshDeviceSingleCardFixture, TensixDataMovementOneFromOneDirectedIdeal) {
    uint32_t test_id = 917;
