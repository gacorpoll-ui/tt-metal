...
def _prepare_one_conv_in_place(
    param_node, conv_args_node, conv_module, device, torch_weight, torch_bias, weights_dtype=ttnn.bfloat8_b
):
    ...
    avoid an extra D2H+H2D round-trip during prep. `weights_dtype` must match
    the Conv2dConfig the consuming TtConv2d/TtnnConv2D wrapper uses at
    runtime — mismatch leads to wrong sharding spec at runtime.
    """
    ...
    common_kwargs = dict(
        ...
        conv_config=ttnn.Conv2dConfig(weights_dtype=weights_dtype),
    )
...
def _prepare_bev_adapter_convs(parameters, model, device):
    ...
    for block_idx in range(len(bev_adapter)):
        ...
        for conv_local_idx in (0, 2):  # 0 = 3x3, 2 = 1x1 (idx 1 is ReLU)
            ...
            _prepare_one_conv_in_place(
                bp_node, ca, torch_conv, device, torch_conv.weight, torch_conv.bias, weights_dtype=ttnn.bfloat16
            )
...
def create_uniad_model_parameters_uniad(model, device=None):
    ...
    # Pre-prepare ResNet Conv2d weights so the first-call "pulling back to
    # host" reprep is avoided.
    _prepare_resnet_backbone_convs(parameters, model, device)
    # FPN pre-prep was measured to eliminate 14 of 38 reprep warnings but
    # to slow per-frame forward by ~0.8 s and cold-call by ~0.9 s, so the
    # helper is kept disabled. The bev_adapter prep helper still crashes
    # with a 5.5 TB DRAM allocation in its current form. Both helpers
    # remain in this file for future investigation but are not called.
    return parameters
