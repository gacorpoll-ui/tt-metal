#!/usr/bin/env bash
# Stability loop for test_prefill_block_loop on a multi-host 32x4 quad BH galaxy.
#
...
#     ./run_prefill_block_loop_stability_multihost.sh <num_iterations> [log_file]
#
# Required env vars:
#   HOSTS                comma-separated MPI host list (no default; must be set)
#
# Overridable env vars (exported or one-shot prefixed):
#   TCP_INTERFACE        tt-run --tcp-interface (default: ens5f0np0)
...
#   DEEPSEEK_V3_HF_MODEL HF model path passed into the bash -c child
#                        (default: /mnt/models/deepseek-ai/DeepSeek-R1-0528)
#   TEST_PATH            pytest file path
#                        (default: models/demos/deepseek_v3_d_p/tests/test_prefill_block_loop.py)
#   TEST_FILTER          pytest -k expression
#                        (default: 'layer3 and device')
...
#     ./run_prefill_block_loop_stability_multihost.sh 20 /tmp/run.log
#
...
LOG_FILE="${2:-stability_prefill_block_loop_multihost_$(date +%Y%m%d_%H%M%S).log}"
...
DEEPSEEK_V3_HF_MODEL="${DEEPSEEK_V3_HF_MODEL:-/mnt/models/deepseek-ai/DeepSeek-R1-0528}"
TEST_PATH="${TEST_PATH:-models/demos/deepseek_v3_d_p/tests/test_prefill_block_loop.py}"
TEST_FILTER="${TEST_FILTER:-layer3 and device}"
...
log "HOSTS:                  ${HOSTS}"
log "TCP interface:          ${TCP_INTERFACE}"
...
for (( i=1; i<=NUM_ITERATIONS; i++ )); do
    ...
    tt-run \
        ...
        bash -c "DEEPSEEK_V3_HF_MODEL=${DEEPSEEK_V3_HF_MODEL} pytest ${TEST_PATH} -k \"${TEST_FILTER}\"" \
        > >(tee -a "${LOG_FILE}") 2>&1 &
    ttrun_pid=$!
