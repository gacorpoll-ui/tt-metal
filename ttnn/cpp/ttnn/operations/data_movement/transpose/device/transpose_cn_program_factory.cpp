...
ProgramDescriptor TransposeCNProgramFactory::create_descriptor(
    const TransposeParams& /*operation_attributes*/, const TransposeInputs& tensor_args, Tensor& output_tensor) {
    const auto& input_tensor = tensor_args.input;
    const auto& input_shape = input_tensor.padded_shape();
    const bool row_major = input_tensor.layout() == Layout::ROW_MAJOR;

    TT_FATAL(input_tensor.storage_type() == StorageType::DEVICE, "Operand to transpose_cn needs to be on device!");
    TT_FATAL(input_tensor.buffer() != nullptr, "Operand to transpose_cn needs to be allocated in a buffer on device!");
    ...
    const uint32_t stick_size = row_major ? page_shape[1] * input_tensor.element_size() : tt::tile_size(cb_data_format);
