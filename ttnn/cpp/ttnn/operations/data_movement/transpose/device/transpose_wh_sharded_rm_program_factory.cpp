...
ProgramDescriptor TransposeWHShardedRMProgramFactory::create_descriptor(
    ...
    uint32_t pack_num_pages = 0;
    uint32_t pack_num_pages_last_col = 0;
    uint32_t pack_num_pages_last_row = 0;
    uint32_t pack_num_pages_last_row_col = 0;
    if ((W % TILE_WIDTH) != 0 && (H % TILE_HEIGHT) != 0) {
        ...
        const auto output_page_size_last_col = TILE_WIDTH * (H % TILE_HEIGHT) * output_tensor.element_size();
        pack_num_pages_last_col = dst_single_tile_size / output_page_size_last_col;
        const auto output_page_size_last_row = TILE_HEIGHT * (W % TILE_WIDTH) * output_tensor.element_size();
        pack_num_pages_last_row = dst_single_tile_size / output_page_size_last_row;
        pack_num_pages_last_row_col = 1;
    } else if ((W % TILE_WIDTH) != 0 && (H % TILE_HEIGHT) == 0) {
    ...
    constexpr uint32_t in_cb_index = tt::CBIndex::c_24;
    const uint32_t num_in_tiles = wt * 2;
    desc.cbs.push_back(CBDescriptor{
    ...
    constexpr uint32_t im_cb_index = tt::CBIndex::c_25;
    ...
    if (ht > 8) {
        constexpr uint32_t im2_cb_index = tt::CBIndex::c_26;
        ...
        constexpr uint32_t out_cb_index = tt::CBIndex::c_27;
        const uint32_t num_out_tiles = ht * 2;
        desc.cbs.push_back(CBDescriptor{
    ...
    std::vector<uint32_t> reader_compile_time_args = {
        ...
        ht,
        H > TILE_HEIGHT ? TILE_HEIGHT : H % TILE_HEIGHT,
        H % TILE_HEIGHT == 0 ? TILE_HEIGHT : H % TILE_HEIGHT,
        wt,
        stick_size_bytes,
        wt * input_tensor.element_size() * TILE_WIDTH,
        H > TILE_HEIGHT ? TILE_HEIGHT : H % TILE_HEIGHT,
        H % TILE_HEIGHT == 0 ? TILE_HEIGHT : H % TILE_HEIGHT,
    };
    ...
    std::vector<uint32_t> writer_compile_time_args = {
        ...
        ht,
        wt,
        W > TILE_WIDTH ? TILE_WIDTH : W % TILE_WIDTH,
        W % TILE_WIDTH == 0 ? TILE_WIDTH : W % TILE_WIDTH,
        H * output_tensor.element_size(),
        ht * output_tensor.element_size() * TILE_HEIGHT,
    };
    ...
    std::vector<uint32_t> compute_compile_time_args = {
        ...
        ht * wt,
        num_hw_blocks_per_core,
        H % TILE_HEIGHT == 0 ? TILE_HEIGHT : H % TILE_HEIGHT,  // last_output_row_num_datums
        pack_num_pages,
        pack_num_pages_last_col,
        pack_num_pages_last_row,
    ...
    compute_desc.defines = {{"SHARDED", "1"}};
