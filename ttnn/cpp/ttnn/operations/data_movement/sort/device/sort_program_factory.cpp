...
SortProgramFactoryCrossCoreDataExchange::Resources SortProgramFactoryCrossCoreDataExchange::prepare_resources(
    const SortParams& /*attributes*/, const SortInputs& tensor_args, std::vector<Tensor>& output_tensors) {
    auto* const device = tensor_args.input_tensor.device();
    ...
    return Resources{.physical_core_lookup_table_tensor = std::move(physical_core_lookup_table_tensor)};
}
...
ProgramDescriptor SortProgramFactoryCrossCoreDataExchange::create_descriptor(
    ...
    TT_FATAL(
        resources.physical_core_lookup_table_tensor.has_value(),
        "physical_core_lookup_table_tensor must be populated by prepare_resources before create_descriptor");
    auto* const physical_core_lookup_table_tensor_buffer = resources.physical_core_lookup_table_tensor->buffer();
    const tt::DataFormat physical_core_lookup_table_cb_data_format =
        tt::tt_metal::datatype_to_dataformat_converter(resources.physical_core_lookup_table_tensor->dtype());
