...
ReshapeViewTiledProgramFactory::Resources ReshapeViewTiledProgramFactory::prepare_resources(
    const ReshapeViewParams& /*operation_attributes*/,
    const ReshapeViewInputs& /*tensor_args*/,
    Tensor& /*tensor_return_value*/) {
    // The mapping tensor is allocated lazily inside create_descriptor() so that the
    // recreate_mapping_tensor flag (excluded from the program hash) can trigger a
    // recompute on the slow path of cache hits as well as cache misses.
    return Resources{};
}

ProgramDescriptor ReshapeViewTiledProgramFactory::create_descriptor(
    const ReshapeViewParams& operation_attributes,
    const ReshapeViewInputs& tensor_args,
    Tensor& tensor_return_value,
    Resources& resources) {
    ...
    // Honour the legacy override behaviour: when the caller asks for a fresh mapping
    // tensor (e.g. shape differs from a previous cache-hit dispatch with the same hash —
    // recreate_mapping_tensor is intentionally not part of the program hash), recompute
    // and overwrite the resource so subsequent dispatches see the new buffer.
    if (operation_attributes.recreate_mapping_tensor || !resources.mapping_tensor.has_value()) {
        resources.mapping_tensor =
            detail::compute_reshape_mapping_host_tensor(
                num_input_pages, num_output_pages, input_shape, output_shape, tile_shape, face_shape)
                .to_device(input_tensor.device());
    }

    Buffer* mapping_buffer = resources.mapping_tensor->buffer();
    ...
    auto mapping_page_size = resources.mapping_tensor->logical_shape()[-1];
    auto mapping_dataformat = datatype_to_dataformat_converter(resources.mapping_tensor->dtype());
    auto mapping_page_size_bytes = mapping_page_size * resources.mapping_tensor->element_size();
    ...
    // Runtime args contain the mapping_buffer address, which can change across cache
    // hits when recreate_mapping_tensor=true.  Skip emplace_runtime_args / BufferBinding
    // here so the framework takes the slow path on cache hits, re-running this function
    // and producing a fresh mapping_buffer address whenever requested.
    ...
    for (auto c : corerange_to_cores(all_cores, std::nullopt)) {
        ...
        reader_desc.runtime_args.emplace_back(
            c, std::vector<uint32_t>{input_buffer->address(), mapping_buffer->address(), page_idx_start, page_idx_end});

        writer_desc.runtime_args.emplace_back(
            c, std::vector<uint32_t>{output_buffer->address(), page_idx_start, page_idx_end});
