...
ProgramDescriptor UntilizeWithUnpaddingMultiCoreNDShardedProgramFactory::create_descriptor(
    ...
    // Note: a "shard plane" here refers to a 2D plane the size of the last 2 dimensions of the shard.
    ...
    // In height-sharded and interleaved cases, the output page is the entire tensor row
    ...
    std::vector<uint32_t> writer_compile_time_args = {
        ...
        num_tiles_per_output_row,
        tile_width,
        output_tensor_width,
        output_tensor_height,
        static_cast<uint32_t>(input.padded_shape().rank())};

    // Due to tensor squeezing from ND to 4D when the input tensor has rank > 4,
    // writer_common_runtime_args will have at most 8 entries.
    KernelDescriptor::RTArgList writer_common_rt_args;
    for (const auto dim : output.padded_shape()) {
        writer_common_rt_args.push_back(static_cast<uint32_t>(dim));
    }
    ...
    // For ND sharded input, we need info on the input buffer distribution
    ...
    writer_desc.emplace_common_runtime_args(writer_common_rt_args);
    ...
    for (const auto& core : ordered_cores_with_data) {
        ...
        // Writer run-time args (writer also reads from input buffer for ND sharded info)
