 ...
 */
FORCE_INLINE GlobalQIndex
decompose_global_q_index(uint32_t linear_index, uint32_t num_q_chunks, uint32_t NQH, bool use_zigzag) {
    const uint32_t remapped = remap_q_index(linear_index, num_q_chunks, use_zigzag);
    return {
