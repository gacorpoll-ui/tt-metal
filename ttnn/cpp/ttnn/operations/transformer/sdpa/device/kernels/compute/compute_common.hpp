    ...
    if constexpr (legacy_compat) {
        for (int d = 0; d < ITERATIONS_HALF_FACE; d++) {
            ...
            if constexpr (DST_ACCUM_MODE || APPROX) {
                sfpi::dst_reg[(dst_index_out - dst_index_in) * SFP_DST_TILE_ROWS] = out;
            } else {
                sfpi::dst_reg[(dst_index_out - dst_index_in) * SFP_DST_TILE_ROWS] =
                    sfpi::reinterpret<sfpi::vFloat>(float_to_fp16b(out, RoundMode::NearestEven));
            }
            sfpi::dst_reg += 2;
        }
    } else {
        for (int d = 0; d < ITERATIONS_HALF_FACE; d++) {
            ...
            if constexpr (APPROX) {
                sfpi::dst_reg[(dst_index_out - dst_index_in) * SFP_DST_TILE_ROWS] =
                    ckernel::sfpu::_sfpu_reciprocal_<0>(in);
            } else {
                if constexpr (DST_ACCUM_MODE) {
                    sfpi::dst_reg[(dst_index_out - dst_index_in) * SFP_DST_TILE_ROWS] =
                        ckernel::sfpu::_sfpu_reciprocal_<2>(in);
                } else {
