...
NeighborPadAsyncMeshWorkloadFactory::Resources NeighborPadAsyncMeshWorkloadFactory::prepare_resources(
...
ProgramDescriptor NeighborPadAsyncMeshWorkloadFactory::create_descriptor(
    const NeighborPadAsyncParams& operation_attributes,
    const NeighborPadAsyncInputs& tensor_args,
    Tensor& tensor_return_value,
    ...
    TT_FATAL(
        ...
        "NeighborPadAsyncMeshWorkloadFactory::create_descriptor requires a mesh dispatch coordinate");
    ...
    if (is_2d) {
        ...
        if (recv_buf_size > 0) {
            desc.cbs.push_back(CBDescriptor{
                ...
                .core_ranges = worker_core_ranges,
                .format_descriptors = {{CBFormatDescriptor{
                    .buffer_index = static_cast<uint8_t>(recv_cb_index),
                    .data_format = df,
                    .page_size = page_size,
                }}},
            });
    ...
    if (is_2d) {
        ...
        // CB on W fabric cores (sender CB only; W recv goes direct to DRAM, no L1 recv buffer needed)
        desc.cbs.push_back(CBDescriptor{
            .total_size = cb_num_pages * l1_scratch_cb_page_size_bytes,
    ...
    // Build H fabric reader kernel descriptor (uniform compile args across all H cores).
    // Push it onto desc.kernels NOW so we can index it via a stable KernelHandle below.
    KernelDescriptor h_reader_kernel_desc;
    h_reader_kernel_desc.kernel_source =
        "ttnn/cpp/ttnn/operations/experimental/ccl/neighbor_pad_async/device/kernels/"
        "minimal_default_reader.cpp";
    h_reader_kernel_desc.source_type = KernelDescriptor::SourceType::FILE_PATH;
    h_reader_kernel_desc.core_ranges = worker_core_ranges;
    h_reader_kernel_desc.compile_time_args = {
        sender_cb_index,   // cb_output_id
        is_padding_zeros,  // is_padding_zeros
        page_size};        // stick_size
    TensorAccessorArgs(*input_buffer).append_to(h_reader_kernel_desc.compile_time_args);
    h_reader_kernel_desc.compile_time_args.push_back(is_2d ? 1 : 0);              // use_l1_intermediate
    h_reader_kernel_desc.compile_time_args.push_back(is_2d ? recv_cb_index : 0);  // recv_cb_id
    h_reader_kernel_desc.config = ReaderConfigDescriptor{};
    // Common runtime args: addresses uniform across all H cores → multicast, no per-core
    // loops.  Input/output buffer addresses use Buffer* bindings so the cache-hit fast
    // path patches them directly without re-running create_descriptor().
    {
        KernelDescriptor::RTArgList crta;
        crta.push_back(input_buffer);                                         // [0] input addr
        crta.push_back(output_buffer);                                        // [1] output addr
        crta.push_back(operation_attributes.h_neighbor_semaphore.address());  // [2] h sem addr
        h_reader_kernel_desc.emplace_common_runtime_args(crta);
    }
    desc.kernels.push_back(std::move(h_reader_kernel_desc));
    const KernelHandle h_reader_kernel_id = desc.kernels.size() - 1;

    // Build H fabric writer kernel descriptor (uniform compile args across all H cores)
    KernelDescriptor h_writer_kernel_desc;
    h_writer_kernel_desc.kernel_source =
        "ttnn/cpp/ttnn/operations/experimental/ccl/neighbor_pad_async/device/kernels/"
        "minimal_default_writer.cpp";
    h_writer_kernel_desc.source_type = KernelDescriptor::SourceType::FILE_PATH;
    h_writer_kernel_desc.core_ranges = worker_core_ranges;
    h_writer_kernel_desc.compile_time_args = {
        sender_cb_index,   // cb_output_id
        is_padding_zeros,  // is_padding_zeros
        page_size};        // stick_size
    TensorAccessorArgs(*output_buffer).append_to(h_writer_kernel_desc.compile_time_args);
    h_writer_kernel_desc.compile_time_args.push_back(is_2d ? 1 : 0);              // use_l1_intermediate
    h_writer_kernel_desc.compile_time_args.push_back(is_2d ? recv_cb_index : 0);  // recv_cb_id
    h_writer_kernel_desc.compile_time_args.push_back(is_2d ? 1 : 0);              // handle_incoming_writes
    h_writer_kernel_desc.compile_time_args.push_back(0);                          // is_w_fabric_writer (false for H)
    h_writer_kernel_desc.compile_time_args.push_back(operation_attributes.ring_size);  // ring_size
    h_writer_kernel_desc.config = WriterConfigDescriptor{};
    {
        KernelDescriptor::RTArgList crta;
        crta.push_back(input_buffer);                                         // [0] input addr
        crta.push_back(output_buffer);                                        // [1] output addr
        crta.push_back(operation_attributes.h_neighbor_semaphore.address());  // [2] h sem addr
        crta.push_back(operation_attributes.barrier_semaphore.address());     // [3] barrier sem addr
        h_writer_kernel_desc.emplace_common_runtime_args(crta);
    }
    desc.kernels.push_back(std::move(h_writer_kernel_desc));
    const KernelHandle h_writer_kernel_id = desc.kernels.size() - 1;
    ...
    for (uint32_t link = 0; link < num_links; link++) {
        ...
        for (uint32_t direction = 0; direction < num_directions; direction++) {
            ...
            // Fabric connection args.  The ProgramDescriptor specialization of
            // append_fabric_connection_rt_args() indexes into desc.kernels by KernelHandle.
            if (direction) {
    ...
    // Local copy workers on cores not used by fabric: AllCores - FabricCores
    {
        ...
        if (!local_copy_cores.empty()) {
            ...
            // Build consolidated local copy reader kernel descriptor (uniform compile args)
            KernelDescriptor local_reader_kernel_desc;
            local_reader_kernel_desc.kernel_source =
                "ttnn/cpp/ttnn/operations/experimental/ccl/neighbor_pad_async/device/kernels/local_copy_reader.cpp";
            local_reader_kernel_desc.source_type = KernelDescriptor::SourceType::FILE_PATH;
            local_reader_kernel_desc.core_ranges = local_copy_cores;
            local_reader_kernel_desc.compile_time_args = {sender_cb_index, page_size};
            TensorAccessorArgs(*input_buffer).append_to(local_reader_kernel_desc.compile_time_args);
            local_reader_kernel_desc.config = ReaderConfigDescriptor{};
            {
                KernelDescriptor::RTArgList crta;
                crta.push_back(input_buffer);             // CRTA[0]
                crta.push_back(output_buffer);            // CRTA[1] (unused by reader, reserved for consistency)
                crta.push_back(0u);                       // CRTA[2]: stick_start_id (always 0)
                crta.push_back(input_halo_dim_size);      // CRTA[3]
                crta.push_back(num_sticks_per_halo_dim);  // CRTA[4]: num_sticks_to_read
                crta.push_back(num_sticks_per_halo_dim);  // CRTA[5]: num_sticks_per_halo_dim
                local_reader_kernel_desc.emplace_common_runtime_args(crta);
            }
            desc.kernels.push_back(std::move(local_reader_kernel_desc));
            const KernelHandle local_reader_kernel_id = desc.kernels.size() - 1;

            // Build consolidated local copy writer kernel descriptor (uniform compile args)
            KernelDescriptor local_writer_kernel_desc;
            local_writer_kernel_desc.kernel_source =
                "ttnn/cpp/ttnn/operations/experimental/ccl/neighbor_pad_async/device/kernels/local_copy_writer.cpp";
            local_writer_kernel_desc.source_type = KernelDescriptor::SourceType::FILE_PATH;
            local_writer_kernel_desc.core_ranges = local_copy_cores;
            local_writer_kernel_desc.compile_time_args = {sender_cb_index, page_size};
            TensorAccessorArgs(*output_buffer).append_to(local_writer_kernel_desc.compile_time_args);
            local_writer_kernel_desc.config = WriterConfigDescriptor{};
            ...
            {
                KernelDescriptor::RTArgList crta;
                crta.push_back(input_buffer);                       // CRTA[0] (unused by writer, reserved)
                crta.push_back(output_buffer);                      // CRTA[1]
                crta.push_back(writer_stick_start_id);              // CRTA[2]
                crta.push_back(input_halo_dim_size);                // CRTA[3]
                crta.push_back(output_halo_dim_size);               // CRTA[4]
                crta.push_back(operation_attributes.padding_left);  // CRTA[5]
                crta.push_back(writer_num_sticks_to_read);          // CRTA[6]
                crta.push_back(output_num_sticks_per_halo_dim);     // CRTA[7]
                crta.push_back(operation_attributes.logical_h);     // CRTA[8]
                crta.push_back(mask_device_h_offset);               // CRTA[9]
                crta.push_back(t_front_pad_stick_offset);           // CRTA[10]
                local_writer_kernel_desc.emplace_common_runtime_args(crta);
            }
            desc.kernels.push_back(std::move(local_writer_kernel_desc));
            const KernelHandle local_writer_kernel_id = desc.kernels.size() - 1;
    ...
    if (is_2d) {
        ...
        // Build consolidated W fabric reader kernel descriptor (uniform compile args across all W cores)
        KernelDescriptor w_reader_kernel_desc;
        w_reader_kernel_desc.kernel_source =
            "ttnn/cpp/ttnn/operations/experimental/ccl/neighbor_pad_async/device/kernels/"
            "phase2_w_reader.cpp";
        w_reader_kernel_desc.source_type = KernelDescriptor::SourceType::FILE_PATH;
        w_reader_kernel_desc.core_ranges = w_fabric_core_range;
        w_reader_kernel_desc.compile_time_args = {
            sender_cb_index,   // cb_output_id
            is_padding_zeros,  // is_padding_zeros
            page_size};        // stick_size
        TensorAccessorArgs(*output_buffer).append_to(w_reader_kernel_desc.compile_time_args);
        TensorAccessorArgs(*input_buffer).append_to(w_reader_kernel_desc.compile_time_args);
        w_reader_kernel_desc.config = ReaderConfigDescriptor{};
        {
            KernelDescriptor::RTArgList crta;
            crta.push_back(output_buffer);                                        // [0] output addr
            crta.push_back(operation_attributes.barrier_semaphore.address());     // [1] barrier sem
            crta.push_back(operation_attributes.w_neighbor_semaphore.address());  // [2] w sem
            crta.push_back(input_buffer);                                         // [3] input addr
            w_reader_kernel_desc.emplace_common_runtime_args(crta);
        }
        desc.kernels.push_back(std::move(w_reader_kernel_desc));
        const KernelHandle w_reader_kernel_id = desc.kernels.size() - 1;

        // Build consolidated W fabric writer kernel descriptor (uniform compile args across all W cores)
        KernelDescriptor w_writer_kernel_desc;
        w_writer_kernel_desc.kernel_source =
            "ttnn/cpp/ttnn/operations/experimental/ccl/neighbor_pad_async/device/kernels/"
            "minimal_default_writer.cpp";
        w_writer_kernel_desc.source_type = KernelDescriptor::SourceType::FILE_PATH;
        w_writer_kernel_desc.core_ranges = w_fabric_core_range;
        w_writer_kernel_desc.compile_time_args = {
            sender_cb_index,   // cb_output_id
            is_padding_zeros,  // is_padding_zeros
            page_size};        // stick_size
        TensorAccessorArgs(*output_buffer).append_to(w_writer_kernel_desc.compile_time_args);
        w_writer_kernel_desc.compile_time_args.push_back(0);  // use_l1_intermediate (direct-to-DRAM for W)
        w_writer_kernel_desc.compile_time_args.push_back(0);  // recv_cb_id (unused)
        w_writer_kernel_desc.compile_time_args.push_back(0);  // handle_incoming_writes (data goes direct to DRAM)
        w_writer_kernel_desc.compile_time_args.push_back(1);  // is_w_fabric_writer (W writer: true)
        w_writer_kernel_desc.compile_time_args.push_back(w_ring_size);  // ring_size
        w_writer_kernel_desc.config = WriterConfigDescriptor{};
        {
            KernelDescriptor::RTArgList crta;
            crta.push_back(output_buffer);                                        // [0] output addr (as 'input')
            crta.push_back(output_buffer);                                        // [1] output addr
            crta.push_back(operation_attributes.w_neighbor_semaphore.address());  // [2] w sem
            ...
            crta.push_back(operation_attributes.h_neighbor_semaphore.address());  // [3] h sem
            w_writer_kernel_desc.emplace_common_runtime_args(crta);
        }
        desc.kernels.push_back(std::move(w_writer_kernel_desc));
        const KernelHandle w_writer_kernel_id = desc.kernels.size() - 1;
