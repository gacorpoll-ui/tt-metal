...
void kernel_main() {
    ...
    // Exit semaphore exchange
    {
        ...
        DPRINT << "Combine writer[" << linearized_mesh_coord << "]: exit send begin (sem_pre=" << *exit_sem_ptr << ")"
               << ENDL();
        ...
        DPRINT << "Combine writer[" << linearized_mesh_coord << "]: exit send done; waiting (sem_now=" << *exit_sem_ptr
               << ")" << ENDL();
