...
CombineProgramFactory::Resources CombineProgramFactory::prepare_resources(
...
tt::tt_metal::ProgramDescriptor CombineProgramFactory::create_descriptor(
    const CombineParams& operation_attributes,
    const CombineInputs& tensor_args,
    ttnn::Tensor& tensor_return_value,
    ...
    TT_FATAL(
        ...
        "CombineProgramFactory::create_descriptor requires a mesh dispatch coordinate");
    ...
    if (init_zeros) {
        for (uint32_t idle_idx = 0; idle_idx < num_idle_cores; idle_idx++) {
            ...
            // Each idle core signals all sender cores.  Push output_tensor's
            // buffer first as Buffer* so the framework records a BufferBinding
