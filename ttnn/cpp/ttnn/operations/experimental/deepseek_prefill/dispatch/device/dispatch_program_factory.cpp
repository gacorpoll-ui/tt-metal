...
DispatchProgramFactory::Resources DispatchProgramFactory::prepare_resources(
    const DispatchParams& operation_attributes,
    const DispatchInputs& tensor_args,
    DispatchProgramFactory::tensor_return_value_t& /*tensor_return_value*/) {
    ...
    Resources resources;
    resources.init_semaphore = std::move(init_barrier_semaphore);
    resources.exit_semaphore = std::move(exit_barrier_semaphore);
    resources.cross_device_semaphore = std::move(final_barrier_semaphore);
    return resources;
}

tt::tt_metal::ProgramDescriptor DispatchProgramFactory::create_descriptor(
    const DispatchParams& operation_attributes,
    const DispatchInputs& tensor_args,
    DispatchProgramFactory::tensor_return_value_t& tensor_return_value,
    ...
    TT_FATAL(
        ...
        "DispatchProgramFactory::create_descriptor requires a mesh dispatch coordinate");
    const ttnn::MeshCoordinate mesh_coordinate = mesh_dispatch_coordinate.value();
    const GlobalSemaphore& init_semaphore = workload_resources.init_semaphore.value();
    const GlobalSemaphore& exit_semaphore = workload_resources.exit_semaphore.value();
    const GlobalSemaphore& cross_device_semaphore = workload_resources.cross_device_semaphore.value();
