...
// ProgramDescriptor-based factory for the (height- and block-) sharded Conv2d variant.
//
// Lives in the multi-variant `Conv2dDeviceOperation::program_factory_t` next to the descriptor
// based Conv2dWidthShardedProgramFactory. The framework's adapter dispatches to the descriptor
...
struct Conv2dShardedProgramFactory {
    ...
    // Builds the ProgramDescriptor for the sharded conv2d program (height-sharded, block-sharded
    // and 1D depthwise variants). `resources` holds the DeviceStorage produced by
    // prepare_resources(); its buffer is used both as the backing buffer for the (optional)
    // L1-resident READER_INDICES CB and to embed the DRAM-resident config buffer address into
    // the activation reader kernel's compile-time args.
