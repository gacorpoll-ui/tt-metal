...
static ProgramDescriptor create_program_mcast_in0_in1_descriptor(
    ...
    if (in0_block_sharded) {
        CoreCoord in0_shard_grid = in0_tensor.legacy_shard_spec()->grid.bounding_box().grid_size();
    ...
    bool in1_is_dram = in1_tensor.memory_config().is_dram();
    ...
    for (const auto& core : cores) {
        ...
        if (in0_block_sharded) {
...

            {
                ...
                in0_args[0] = std::cref(in0_tensor);
        ...
        if (in0_idx < num_blocks_y and in1_idx < num_blocks_x) {
            ...
            if (in0_idx == 0) {
                ...
                mm_in1_sender_writer_args.push_back(
                    bias_tensor.has_value() ? (std::uint32_t)per_core_N * in1_idx : 0);  // in1_tensor_start_tile_id
                if (!output_is_sharded) {
                    if (in1_idx == in1_end_idx) {  // right cores when no transpose_mcast
                ...
                }
                {
                    ...
                    in1_sender_variant[0] = std::cref(in1_tensor);
                    in1_sender_variant[7] = std::cref(out_tensor);
                    if (bias_tensor.has_value()) {
                        in1_sender_variant[18] = std::cref(bias_tensor.value());
                    }
...

                {
                    ...
                    in1_recv_variant[2] = std::cref(out_tensor);
                    // left half
                    if (core.x <= half_core || (transpose_mcast and core.y == start_core_y)) {
...
void override_runtime_arguments_impl(
    ...
    ttsl::optional_reference<const tt::tt_metal::MeshTensor> bias_mt;
    if (optional_input_tensors.at(0).has_value()) {
        bias_mt = optional_input_tensors.at(0)->mesh_tensor();
    }
    ...
    // in0 sender
    if (src0_sharded) {
        UpdateDynamicCircularBufferAddress(program, cb_src2, src_tensor_a);
    } else {
    ...
    for (const auto& core : in1_sender_cores) {
        ...
        if (bias_mt.has_value()) {
            writer_runtime_args[18] = bias_mt->address();
        }
    }
    ...
    if (out_sharded) {
        UpdateDynamicCircularBufferAddress(program, cb_output, dst_tensor);
    }
}
}  // namespace reuse_mcast_optimized_helpers
...
matmul_multi_core_reuse_mcast_2d_optimized_(
    ...
    ttsl::optional_reference<const tt::tt_metal::MeshTensor> bias_tensor_mesh;
    ...
    if (bias.has_value()) {
        ...
        bias_tensor_mesh = c.mesh_tensor();
    ...
    const tt::tt_metal::MeshTensor& in0_mt = a.mesh_tensor();
    const tt::tt_metal::MeshTensor& in1_mt = b.mesh_tensor();
    const tt::tt_metal::MeshTensor& out_mt = output.mesh_tensor();

    auto in0_buffer_size = in0_mt.mesh_buffer().device_local_size();
    auto in1_buffer_size = in1_mt.mesh_buffer().device_local_size();
    TT_FATAL(
    ...
    ////////////////////////////////////////////////////////////////////////////
    TT_FATAL(output.storage_type() == StorageType::DEVICE, "Output tensor must be on device!");
...
ProgramDescriptor MatmulMultiCoreReuseMcast2DProgramFactory::create_descriptor(
    ...
    ttsl::optional_reference<const tt::tt_metal::MeshTensor> bias_tensor_mesh;
    if (bias.has_value()) {
        bias_tensor_mesh = bias.value().mesh_tensor();
