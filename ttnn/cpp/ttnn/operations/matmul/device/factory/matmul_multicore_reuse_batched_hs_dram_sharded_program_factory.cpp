...
static ProgramDescriptor create_program_batch_sharded_descriptor(
    ...
    uint32_t in0_shard_tiles = in0_tensor.legacy_shard_spec()->shape[0] / in0_tile.get_tile_shape()[0] *
                               in0_tensor.legacy_shard_spec()->shape[1] / in0_tile.get_tile_shape()[1];
    ...
    uint32_t out_shard_tiles = out_tensor.legacy_shard_spec()->shape[0] / output_tile.get_tile_shape()[0] *
                               out_tensor.legacy_shard_spec()->shape[1] / output_tile.get_tile_shape()[1];
    ...
    for (uint32_t worker_idx = 0; worker_idx < all_worker_cores_ordered.size(); ++worker_idx) {
        ...
        if (bias_tensor.has_value()) {
            in1_writer_args.push_back(bias_tensor.value());
        } else {
