...
# shard_tiles() / w2_shard_tiles() formulas below. The hand-rolled BH N=8/16
# tables previously kept here are redundant: shard_tiles(N, c, n_cores) produces
# the same distribution. See PR #43932 commit message for the math sanity check.
####################################################################################################
...
def get_weight_core_shard_maps(mesh_device, hidden_size: int, intermediate_size: int):
    ...
    for ring_pos in range(target_ring_size):
        ...
        w0_w1_tiles = shard_tiles(Nt, ring_pos, target_ring_size)
        w0_w1_shard_map.append(w0_w1_tiles)

        w2_tiles = w2_shard_tiles(Ht, ring_pos, Nt, target_ring_size)
