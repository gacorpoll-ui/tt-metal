...
FIX_AV_FIRES=$(grep -cE 'FIX AV[: ]+#42429|FIX AV:? #42429|FIX AY/AV.*Skipping all remaining' "$CLEAN" 2>/dev/null; :)
...
# FIX BE (#42429 — STALE STATE CLEARING): compile_and_configure_fabric entry clearing stale per-cycle state.
# Log: "compile_and_configure_fabric: FIX BE (#42429) clearing stale per-cycle state:"
FIX_BE_STALE_CLEAR=$(grep -cE 'FIX BE.*clearing stale per-cycle state' "$CLEAN" 2>/dev/null; :)
...
# FIX BN (#42429): fw_launch_addr=0 moved to post-heartbeat-confirm path in teardown.
# FIX BN is structural (no dedicated log tag). Detect its absence-of-bug via:
#   - ROM hang: "did not exit ROM within" — if FIX BN regresses, these appear.
#   - fw_launch_addr clear confirmation: FIX PE log + FIX XZ heartbeat-confirm path (both clear fw_launch_addr).
FIX_BN_ROM_HANG=$(grep -cE 'did not exit ROM within' "$CLEAN" 2>/dev/null; :)
FIX_BN_FW_LAUNCH_CLEAR=$(grep -cE 'cleared fw_launch_addr|FIX PE.*clearing fw_launch_addr' "$CLEAN" 2>/dev/null; :)
# FIX BR (#42429): Write fw_launch_addr before deassert in FIX AC paths (risc_firmware_initializer.cpp).
# FIX BR is structural (no dedicated log tag). Detect its absence-of-bug via FIX AC/PG timeouts:
#   - If FIX BR regresses, "FIX PG" fires because FIX AC heartbeat poll times out.
FIX_BR_REGRESSION=$(grep -cE 'FIX PG.*fw_launch_addr|heartbeat poll.*timeout.*FIX AC' "$CLEAN" 2>/dev/null; :)
# FIX BS (#42429): Accept BASE_FW_HEARTBEAT_SIGNATURE (0xABCD) immediately in UMD eth_heartbeat_running().
# FIX BS is in UMD (topology_discovery.cpp). Detect regression:
#   - "Timed out waiting for ETH heartbeat on core ... to advance. Stuck at 0xabcd" = FIX BS regression
FIX_BS_HEARTBEAT_STUCK_ABCD=$(grep -cE 'Timed out waiting for ETH heartbeat.*to advance.*Stuck at 0xabcd' "$CLEAN" 2>/dev/null; :)
# FIX BZ (#42429): write fw_launch_addr_value instead of 0 in FIX PE and FIX BN paths.
# Log: "FIX BZ — wrote fw_launch_addr_value (not 0)"
FIX_BZ_FIRES=$(grep -cE 'FIX BZ.*fw_launch_addr_value|wrote fw_launch_addr_value' "$CLEAN" 2>/dev/null; :)
# FIX CA (#42429): write-only reset of non-MMIO ETH ERISCs BEFORE FIX AC MMIO reset.
# Log: "teardown: FIX CA (#42429) — write-only reset of N non-MMIO ETH ERISC device(s)"
FIX_CA_FIRES=$(grep -cE 'FIX CA.*write-only reset|write-only reset.*non-MMIO ETH ERISC' "$CLEAN" 2>/dev/null; :)
# FIX CA success: "FIX CA — all N non-MMIO ETH ERISCs write-only reset via MMIO relay"
FIX_CA_SUCCESS=$(grep -cE 'FIX CA.*all.*non-MMIO ETH ERISCs write-only reset' "$CLEAN" 2>/dev/null; :)
# FIX CA partial: "FIX CA — N/M non-MMIO ETH ERISCs write-only reset succeeded (K failed)"
FIX_CA_PARTIAL=$(grep -cE 'FIX CA.*succeeded.*failed' "$CLEAN" 2>/dev/null; :)
# FIX CA per-core failure: "FIX CA — write-only reset of non-MMIO ETH core ... failed"
FIX_CA_CORE_FAIL=$(grep -cE 'FIX CA.*write-only reset of non-MMIO ETH core.*failed' "$CLEAN" 2>/dev/null; :)
# FIX CB (#42429): 50ms delay after FIX CA non-MMIO resets.
# Log: "teardown: FIX CB (#42429) — 50ms delay after FIX CA"
FIX_CB_FIRES=$(grep -cE 'FIX CB.*50ms delay|50ms delay after FIX CA' "$CLEAN" 2>/dev/null; :)
# FIX CC (#42429): write fw_launch_addr_value for non-MMIO channels in FIX CA (mirrors FIX BR for MMIO).
# No success log (silent on success). Failure log: "teardown: FIX CC — fw_launch_addr_value relay write"
FIX_CC_FAIL=$(grep -cE 'FIX CC.*fw_launch_addr_value relay write.*failed' "$CLEAN" 2>/dev/null; :)
# FIX CD (#42429): write fw_launch_addr_value instead of 0 in FIX PD quiesce paths (device.cpp).
# Log: "FIX PD/CD (GAP-B)"
FIX_CD_FIRES=$(grep -cE 'FIX PD/CD' "$CLEAN" 2>/dev/null; :)
# FIX CE (#42429): write fw_launch_addr_value before deassert in teardown force-reset path
# (fabric_firmware_initializer.cpp).  Success: "FIX CE (#42429): Device".  Failure: "FIX CE (#42429): fw_launch_addr_value write"
FIX_CE_SUCCESS=$(grep -cE 'FIX CE \(#42429\): Device.*wrote fw_launch_addr_value' "$CLEAN" 2>/dev/null; :)
FIX_CE_FAIL=$(grep -cE 'FIX CE \(#42429\): fw_launch_addr_value write.*failed' "$CLEAN" 2>/dev/null; :)
# FIX CF (#42429): write fw_launch_addr_value before deassert in FIX RR-NM step 2 and FIX AY paths.
# Success: "FIX CF (#42429)".  Failure: "FIX CF (#42429).*failed"
FIX_CF_SUCCESS=$(grep -cE 'FIX CF \(#42429\).*wrote fw_launch_addr_value' "$CLEAN" 2>/dev/null; :)
FIX_CF_FAIL=$(grep -cE 'FIX CF \(#42429\).*failed' "$CLEAN" 2>/dev/null; :)
# FIX CG (#42429): dual relay failure — FIX CA and FIX AY both failed for the same non-MMIO device.
# Channel is unreachable via relay in both passes; left for next session's FIX RR-NM.
# Pattern: "FIX CG (#42429)"
FIX_CG_DUAL_FAIL=$(grep -cE 'FIX CG \(#42429\)' "$CLEAN" 2>/dev/null; :)
# FIX CH (#42429): MMIO ERISC command-queue flush timeout during write_core_immediate relay write.
# Flush polls LOCAL MMIO ERISC queue — does NOT confirm non-MMIO delivery (fire-and-forget).
# Pattern: "FIX CH (#42429)"
FIX_CH_FLUSH_TIMEOUT=$(grep -cE 'FIX CH \(#42429\)' "$CLEAN" 2>/dev/null; :)
# FIX CI (#42429): fw_launch_addr {0} write debug breadcrumbs in dispatch_kernel_initializer.
# Only visible when DEBUG logging is enabled (log_debug level).
# Pattern: "FIX CI (#42429)"
FIX_CI_ZERO_WRITE=$(grep -cE 'FIX CI \(#42429\)' "$CLEAN" 2>/dev/null; :)
# FIX CJ (#42429): bc_deadlock channels bypassed TERMINATE poll — routed to immediate force-reset.
# Pattern: "FIX CJ (#42429)"
FIX_CJ_BYPASS=$(grep -cE 'FIX CJ \(#42429\)' "$CLEAN" 2>/dev/null; :)
# FIX CL (#42429): collective assert-then-deassert in teardown force-reset — ensures simultaneous
# ETH link training by resetting both sides of each link before deasserting either side.
# Pattern: "FIX CL (#42429)"
FIX_CL_COLLECTIVE=$(grep -cE 'FIX CL \(#42429\)' "$CLEAN" 2>/dev/null; :)
FIX_CL_DEASSERT_FAIL=$(grep -cE 'FIX CL deassert_risc_reset_at_core.*failed' "$CLEAN" 2>/dev/null; :)
# FIX CN (#42429): bc_deadlock channels skipped from fabric_firmware_init::teardown force-reset.
# Delegated to RiscFirmwareInitializer::teardown FIX AC/AY instead.
# Pattern: "FIX CN (#42429)"
FIX_CN_SKIP=$(grep -cE 'FIX CN \(#42429\)' "$CLEAN" 2>/dev/null; :)
# FIX CO (#42429): MMIO ETH channels skipped in FIX AC reset loop because their non-MMIO
# peer is a bc_deadlock channel on a relay-broken device (link training would hang).
# Pattern: "FIX CO (#42429)"
FIX_CO_SKIP=$(grep -cE 'FIX CO \(#42429\)' "$CLEAN" 2>/dev/null; :)
# FIX CP (#42429): MMIO peers of ALL channels on FIX CA-failed non-MMIO devices added to
# FIX AC skip set (broader than FIX CO which only covers bc_deadlock channels).
# Pattern: "FIX CP (#42429)"
FIX_CP_SKIP=$(grep -cE 'FIX CP \(#42429\)' "$CLEAN" 2>/dev/null; :)
# FIX CS (#42429): Collective assert-all/deassert-all in FIX RR-NM step 1.
FIX_CS_ASSERT=$(grep -cE 'FIX CS \(#42429\) FIX RR-NM step 1:.*asserting' "$CLEAN" 2>/dev/null; :)
FIX_CS_DEASSERT=$(grep -cE 'FIX CS \(#42429\) FIX RR-NM step 1:.*asserted [0-9]' "$CLEAN" 2>/dev/null; :)
# FIX CT (#42429): Relay-deassert non-MMIO dead channels before Phase C.
FIX_CT_OK=$(grep -cE 'FIX CT \(#42429\) — relay-deassert non-MMIO' "$CLEAN" 2>/dev/null; :)
FIX_CT_SKIP=$(grep -cE 'FIX CT \(#42429\) — MMIO relay dev=.* has no working channels' "$CLEAN" 2>/dev/null; :)
FIX_CT_FAIL=$(grep -cE 'FIX CT \(#42429\) — relay-deassert non-MMIO.*failed' "$CLEAN" 2>/dev/null; :)
# FIX DC (#42429): Parallel poll in Phase D and FIX BH.
FIX_DC_PHASE_D=$(grep -cE 'FIX DC \(#42429\) FIX CS Phase D: parallel-polling' "$CLEAN" 2>/dev/null; :)
FIX_DC_PHASE_D_OK=$(grep -cE 'FIX DC \(#42429\) FIX CS Phase D:.*reached base-UMD' "$CLEAN" 2>/dev/null; :)
FIX_DC_PHASE_D_FAIL=$(grep -cE 'FIX DC \(#42429\) FIX CS Phase D:.*did not reach base-UMD' "$CLEAN" 2>/dev/null; :)
FIX_DC_BH=$(grep -cE 'FIX DC \(#42429\) FIX BH: parallel-polling' "$CLEAN" 2>/dev/null; :)
FIX_DC_BH_OK=$(grep -cE 'FIX DC \(#42429\) FIX BH:.*booted from ROM' "$CLEAN" 2>/dev/null; :)
FIX_DC_BH_FAIL=$(grep -cE 'FIX DC \(#42429\) FIX BH:.*did not exit ROM' "$CLEAN" 2>/dev/null; :)
# FIX DD (#42429): try/catch in FIX BH parallel poll (audit fix).
FIX_DD_FIRES=$(grep -cE 'FIX DD \(#42429\)' "$CLEAN" 2>/dev/null; :)
...
# FIX BC Phase 5b (#42429): simultaneous-handshake deadlock detection.
# When all stuck channels are at REMOTE_HANDSHAKE_COMPLETE and any peer chip IS in the
# quiesce set, FIX BC identifies a simultaneous-handshake deadlock (both sides launched
# EDM simultaneously, both stuck waiting for each other) and sets
# fabric_channels_not_ready_for_traffic_ so FIX AA can GTEST_SKIP.
# Log pattern: "FIX BC (#42429): simultaneous-handshake deadlock detected"
FIX_BC_P5B_FIRES=$(grep -cE 'FIX BC \(#42429\).*simultaneous.handshake deadlock' "$CLEAN" 2>/dev/null; :)
# FIX BD (AUDIT): Enhanced FIX BC logging — logs ALL stuck channels' peer resolution.
FIX_BD_FIRES=$(grep -cE 'FIX BD \(AUDIT\)|FIX BC \(#42429\).*simultaneous.handshake.*channel.s. have peer' "$CLEAN" 2>/dev/null; :)
...
# FIX BY (#42429 — BENIGN ERISC STATE): false-positive corrupt ETH classification fix in pre-init scan.
# Log: "pre-init L1 scan: Device N has N stale-running, N booting/base-firmware, and N corrupt"
# The booting/base-firmware count being > 0 indicates is_benign_erisc_state() fired.
FIX_BY_BENIGN=$(grep -cE 'pre-init L1 scan:.*booting/base-firmware' "$CLEAN" 2>/dev/null; :)
...
# FIX FX (#42429): persistent fixm_init flag — FIX FX-1 in mesh_device (quiesce propagation),
# FIX FX-2 in device.cpp (Phase 5b timeout + Phase 5 kSyncTimeoutMs).
# Log: "FIX FX-1 (#42429)" or "FIX QH-2/FX-2"
FIX_FX_FIRES=$(grep -cE 'FIX FX-1|FIX FX-2|FIX QH-2/FX-2' "$CLEAN" 2>/dev/null; :)
# FIX QH (#42429): Phase 5b excludes P25-CLEAN channels (QH-1) and extends timeout for stale
# base-UMD (QH-2).  QH-1 saves + consumes P25-CLEAN set; QH-2 extends kHealthCheckTimeoutMs.
# Log: "FIX QH-1" or "FIX QH-2"
FIX_QH1_FIRES=$(grep -cE 'FIX QH-1' "$CLEAN" 2>/dev/null; :)
FIX_QH2_FIRES=$(grep -cE 'FIX QH-2' "$CLEAN" 2>/dev/null; :)
# FIX FQ (#42429): dispatch hang remediations — relay target validation (FQ-1), post-init
# ETH FW verification (FQ-2), enhanced diagnostics (FQ-5).
# Log: "FIX FQ-1" or "FIX FQ-2" or "FIX FQ-5"
FIX_FQ1_FIRES=$(grep -cE 'FIX FQ-1' "$CLEAN" 2>/dev/null; :)
FIX_FQ2_FIRES=$(grep -cE 'FIX FQ-2' "$CLEAN" 2>/dev/null; :)
FIX_FQ5_FIRES=$(grep -cE 'FIX FQ-5' "$CLEAN" 2>/dev/null; :)
# FIX P25-CLEAN-V2 (#42429): gate P25-CLEAN skip on peer not in quiesce set.
# Without V2, P25-CLEAN incorrectly skipped channels whose peer WAS in the quiesce set.
# Log: "FIX P25-CLEAN-V2"
FIX_P25CV2_FIRES=$(grep -cE 'FIX P25-CLEAN-V2' "$CLEAN" 2>/dev/null; :)
# FIX BW (#42429): write_core_immediate relay failure guard — catches throws and marks relay broken.
# Log: "FIX BW: write_core_immediate"
FIX_BW_FIRES=$(grep -cE 'FIX BW.*write_core_immediate' "$CLEAN" 2>/dev/null; :)
# FIX BX (#42429): initialize_and_launch_firmware exception guard — catches throws for non-MMIO.
# Log: "FIX BX"
FIX_BX_FIRES=$(grep -cE 'FIX BX.*initialize_and_launch_firmware' "$CLEAN" 2>/dev/null; :)
...
# FIX BG (audit #7): partial force-reset failure logging in teardown() second pass.
# Three log patterns:
#   1. "teardown second-pass: Device N (non-mmio) chan=X ... skipping diagnostic read"
#      — non-MMIO channel whose relay was already confirmed dead; diagnostic read skipped.
#   2. "teardown second-pass: Device N (mmio|non-mmio) chan=X ... skipping assert_risc_reset_at_core"
#      — channel delegated to FIX AC PCIe-reset (relay dead or assert already failed).
#   3. "teardown second-pass: Device N (mmio) chan=X ... did not report base firmware heartbeat"
#      — MMIO channel did not reach base firmware within 3000ms after force-reset (FIX XZ timeout).
FIX_BG_SECOND_PASS_FIRES=$(grep -cE 'teardown second-pass:' "$CLEAN" 2>/dev/null; :)
FIX_BG_RELAY_DEAD_SKIP=$(grep -cE 'teardown second-pass:.*skipping diagnostic read.*relay already confirmed dead' "$CLEAN" 2>/dev/null; :)
FIX_BG_ASSERT_SKIP=$(grep -cE 'teardown second-pass:.*skipping assert_risc_reset_at_core' "$CLEAN" 2>/dev/null; :)
FIX_BG_HB_TIMEOUT=$(grep -cE 'teardown second-pass:.*did not report base firmware heartbeat' "$CLEAN" 2>/dev/null; :)
# FIX VC2 (#42429): Skip channels 14/15 (external UMD links) + coord-translation guard.
# Log: "(FIX VC2 #42429)" or "(FIX VC2-EXT #42429)"
FIX_VC2_FIRES=$(grep -cE 'FIX VC2(-EXT)? #42429' "$CLEAN" 2>/dev/null; :)
# FIX VC3 (#42429): Coordinate-translation failure guard in Phase 5 health check.
# Log: "(FIX VC3 #42429)" or "(FIX VC3b #42429)"
FIX_VC3_FIRES=$(grep -cE 'FIX VC3b? #42429' "$CLEAN" 2>/dev/null; :)
# FIX NZ (#42429): Skip initialize_and_launch_firmware for non-MMIO devices with broken relay.
# Log: "run_launch_phase: FIX NZ — skipping"
FIX_NZ_FIRES=$(grep -cE 'FIX NZ.*skipping initialize_and_launch_firmware' "$CLEAN" 2>/dev/null; :)
# FIX PE (GAP-56): Clear fw_launch_addr for cleanly-terminated channels.
# Log: "FIX PE — clearing fw_launch_addr"
FIX_PE_FIRES=$(grep -cE 'FIX PE.*clearing fw_launch_addr' "$CLEAN" 2>/dev/null; :)
# FIX QD (#42429): Set channels_not_ready after skipping health check on dead non-MMIO.
# Log: "Skipping health check. (#42429 FIX AN / FIX QD)"
FIX_QD_FIRES=$(grep -cE 'FIX AN / FIX QD' "$CLEAN" 2>/dev/null; :)
# FIX RR-NM (#42429): Relay-assisted ERISC reset for non-MMIO dead channels via MMIO PCIe.
# Log: "FIX RR-NM step 1"
FIX_RR_NM_FIRES=$(grep -cE 'FIX RR-NM step 1' "$CLEAN" 2>/dev/null; :)
# FIX TB (#42429): ASIC ID not found in topology mapping during re-discovery.
# Log: "FIX TB (#42429): ASIC id N not found"
FIX_TB_FIRES=$(grep -cE 'FIX TB.*ASIC id.*not found' "$CLEAN" 2>/dev/null; :)
# FIX BQ (#42429): Typed catch for fabric_context-already-torn-down in teardown.
# Log: "FIX BQ typed catch"
FIX_BQ_FIRES=$(grep -cE 'FIX BQ typed catch' "$CLEAN" 2>/dev/null; :)
# FIX BU (#42429): REMOTE_HANDSHAKE_COMPLETE early-exit — defer to Phase 5b.
# Log: "(FIX BU: #42429)"
FIX_BU_FIRES=$(grep -cE 'FIX BU.*#42429' "$CLEAN" 2>/dev/null; :)
# FIX BT (#42429): ROM boot postcode promoted to probe_dead_channels immediately.
# Log: "FIX BT Device N chan=N ROM boot postcode"
FIX_BT_FIRES=$(grep -cE 'FIX BT Device.*ROM boot postcode' "$CLEAN" 2>/dev/null; :)
# FIX EXT2 (#42429): Corrected external channel detection via get_ethernet_connections().
# Log: "(FIX EXT2 #42429)"
FIX_EXT2_FIRES=$(grep -cE 'FIX EXT2 #42429' "$CLEAN" 2>/dev/null; :)
# FIX RZ3 (#42429): Persistent companion flag persists stale-base-UMD guard past ring-sync.
# Log: "FIX RZ3 persists the guard"
FIX_RZ3_FIRES=$(grep -cE 'FIX RZ3 persists the guard' "$CLEAN" 2>/dev/null; :)
# FIX RZ4 (#42429): Transient channels_not_ready cleared after Phase 5b success.
# Log: "FIX RZ4 — transient" or "FIX RZ4 — all channels confirmed"
FIX_RZ4_FIRES=$(grep -cE 'FIX RZ4' "$CLEAN" 2>/dev/null; :)
# FIX AY-C (#42429): Per-pair MMIO relay readiness guard in teardown.
# Log: "FIX AY-C — could not read peer" or "FIX AY-C — MMIO peer"
FIX_AY_C_FIRES=$(grep -cE 'FIX AY-C' "$CLEAN" 2>/dev/null; :)
# FIX AQ-2 (#42429): UMD topology discovery skip for unreachable devices.
# Log: "FIX AQ-2: Skipping ETH connection to unreachable device"
FIX_AQ2_FIRES=$(grep -cE 'FIX AQ-2.*Skipping ETH connection' "$CLEAN" 2>/dev/null; :)
...
# FIX E2 (#42429): dead-relay device skipped configure_fabric + dispatch kernel init.
# Log: "Dispatch kernel initialization will be skipped (#42429 FIX E2)"
FIX_E2_FIRES=$(grep -cE 'FIX E2' "$CLEAN" 2>/dev/null; :)
# FIX G (#42429): dead-relay device skipped router sync / health verification.
# Log: "(#42429 FIX G)"
FIX_G_FIRES=$(grep -cE 'FIX G[^A-Z]' "$CLEAN" 2>/dev/null; :)
# FIX H (#42429): MMIO device has all ETH channels dead after configure_fabric_cores.
# Log: "(FIX H #42429)"
FIX_H_FIRES=$(grep -cE 'FIX H[^A-Z]' "$CLEAN" 2>/dev/null; :)
# FIX I2 (#42429): MMIO device's master router connects to dead-relay peer (transitive).
# Log: "(#42429 FIX I2)" or "(#42429 FIX I2 transitive)"
FIX_I2_FIRES=$(grep -cE 'FIX I2' "$CLEAN" 2>/dev/null; :)
# FIX U (#42429): non-MMIO master channels not ready after Phase 5 poll.
# Log: "(FIX U/V: #42429)"
FIX_U_FIRES=$(grep -cE 'FIX U[^A-Z]|FIX U/V' "$CLEAN" 2>/dev/null; :)
...
if [ "${FIX_BN_ROM_HANG:-0}" -gt 0 ]; then
    echo "  => [FIX BN REGRESSION] ${FIX_BN_ROM_HANG} channel(s) 'did not exit ROM' — fw_launch_addr=0 race may have returned."
    echo "     FIX BN moved fw_launch_addr=0 write to post-heartbeat-confirm; this pattern means the fix may be incomplete."
fi
if [ "${FIX_BN_FW_LAUNCH_CLEAR:-0}" -gt 0 ]; then
    echo "  => [FIX BN/PE] ${FIX_BN_FW_LAUNCH_CLEAR} fw_launch_addr clear event(s) — post-heartbeat or clean-TERMINATE path executed."
fi
if [ "${FIX_BS_HEARTBEAT_STUCK_ABCD:-0}" -gt 0 ]; then
    echo "  => [FIX BS REGRESSION] ${FIX_BS_HEARTBEAT_STUCK_ABCD} ETH heartbeat 'stuck at 0xabcd' event(s)."
    echo "     FIX BS should accept 0xABCD immediately. If this fires, UMD eth_heartbeat_running() may not have FIX BS."
fi
if [ "${FIX_BZ_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX BZ] ${FIX_BZ_FIRES} fw_launch_addr_value write(s) (replaces 0-write in FIX PE/BN paths) — ERISC stays bootable after any subsequent deassert."
fi
if [ "${FIX_CA_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX CA] ${FIX_CA_FIRES} write-only reset event(s) — non-MMIO ETH ERISCs reset BEFORE FIX AC MMIO reset."
    echo "     Purpose: break ETH link-training deadlock by ensuring non-MMIO side boots first."
    if [ "${FIX_CA_SUCCESS:-0}" -gt 0 ]; then
        echo "     => FIX CA full success: ${FIX_CA_SUCCESS} event(s) — all non-MMIO ERISCs reset."
    fi
    if [ "${FIX_CA_PARTIAL:-0}" -gt 0 ]; then
        echo "     => FIX CA PARTIAL: ${FIX_CA_PARTIAL} event(s) — some non-MMIO resets failed; FIX AY may still be needed."
    fi
    if [ "${FIX_CA_CORE_FAIL:-0}" -gt 0 ]; then
        echo "     => FIX CA per-core failures: ${FIX_CA_CORE_FAIL} — relay write-only reset threw on specific cores."
    fi
fi
if [ "${FIX_CB_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX CB] ${FIX_CB_FIRES} 50ms delay event(s) after FIX CA — non-MMIO ERISCs given time to begin ROM boot."
fi
if [ "${FIX_CC_FAIL:-0}" -gt 0 ]; then
    echo "  => [FIX CC] WARNING: ${FIX_CC_FAIL} fw_launch_addr_value relay write failure(s) to non-MMIO — non-MMIO may enter link-training wait."
fi
if [ "${FIX_CD_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX CD] WARNING: ${FIX_CD_FIRES} fw_launch_addr write failure(s) in FIX PD quiesce path (device.cpp)."
fi
if [ "${FIX_CE_SUCCESS:-0}" -gt 0 ] || [ "${FIX_CE_FAIL:-0}" -gt 0 ]; then
    echo "  => [FIX CE] fw_launch_addr pre-write in teardown force-reset: ${FIX_CE_SUCCESS:-0} success, ${FIX_CE_FAIL:-0} failure(s)."
fi
if [ "${FIX_CF_SUCCESS:-0}" -gt 0 ] || [ "${FIX_CF_FAIL:-0}" -gt 0 ]; then
    echo "  => [FIX CF] fw_launch_addr pre-write in FIX RR-NM/AY paths: ${FIX_CF_SUCCESS:-0} success, ${FIX_CF_FAIL:-0} failure(s)."
fi
if [ "${FIX_CG_DUAL_FAIL:-0}" -gt 0 ]; then
    echo "  => [FIX CG] WARNING: ${FIX_CG_DUAL_FAIL} non-MMIO channel(s) unreachable after DUAL relay failure (FIX CA + FIX AY both failed) — left for next session FIX RR-NM."
fi
if [ "${FIX_CH_FLUSH_TIMEOUT:-0}" -gt 0 ]; then
    echo "  => [FIX CH] WARNING: ${FIX_CH_FLUSH_TIMEOUT} MMIO ERISC command-queue flush timeout(s) during relay write — MMIO relay ERISC under load or crashed."
fi
if [ "${FIX_CI_ZERO_WRITE:-0}" -gt 0 ]; then
    echo "  => [FIX CI] ${FIX_CI_ZERO_WRITE} dispatch fw_launch_addr {0} write breadcrumb(s) (DEBUG level — only present with TT_LOGGER_LEVEL=debug)."
fi
if [ "${FIX_CJ_BYPASS:-0}" -gt 0 ]; then
    echo "  => [FIX CJ] ${FIX_CJ_BYPASS} bc_deadlock channel(s) bypassed 5s TERMINATE poll — routed directly to force-reset (simultaneous-handshake deadlock, per FIX BH/BC)."
fi
if [ "${FIX_CL_COLLECTIVE:-0}" -gt 0 ]; then
    echo "  => [FIX CL] Collective assert-then-deassert: ${FIX_CL_COLLECTIVE} event(s) — both ETH link sides reset simultaneously to avoid link-training deadlock."
fi
if [ "${FIX_CL_DEASSERT_FAIL:-0}" -gt 0 ]; then
    echo "  => [FIX CL] WARNING: ${FIX_CL_DEASSERT_FAIL} deassert failure(s) in collective Phase 2 — ERISC may be halted."
fi
if [ "${FIX_CN_SKIP:-0}" -gt 0 ]; then
    echo "  => [FIX CN] ${FIX_CN_SKIP} bc_deadlock channel(s) skipped from fabric_firmware_init force-reset — delegated to RiscFirmwareInitializer FIX AC/AY."
fi
if [ "${FIX_CO_SKIP:-0}" -gt 0 ]; then
    echo "  => [FIX CO] ${FIX_CO_SKIP} MMIO ETH channel(s) skipped in FIX AC reset (non-MMIO bc_deadlock peer unreachable via broken relay — link training would hang at 0x49705180)."
fi
if [ "${FIX_CP_SKIP:-0}" -gt 0 ]; then
    echo "  => [FIX CP] ${FIX_CP_SKIP} MMIO channel(s) added to FIX AC skip set from FIX CA-failed non-MMIO device(s) — broader skip than FIX CO."
fi
if [ "${FIX_CS_ASSERT:-0}" -gt 0 ] || [ "${FIX_CS_DEASSERT:-0}" -gt 0 ]; then
    echo "  => [FIX CS] Collective assert-all/deassert-all for dead MMIO channels: ${FIX_CS_ASSERT:-0} assert event(s), ${FIX_CS_DEASSERT:-0} deassert event(s)."
    echo "     FIX CS replaces serial per-channel reset with simultaneous assert → pause → deassert so ETH peers"
    echo "     reboot together (preventing link-training deadlock where one peer is stuck waiting for the other)."
fi
if [ "${FIX_CT_OK:-0}" -gt 0 ] || [ "${FIX_CT_SKIP:-0}" -gt 0 ] || [ "${FIX_CT_FAIL:-0}" -gt 0 ]; then
    echo "  => [FIX CT] Relay-deassert non-MMIO dead channels via working MMIO relay BEFORE Phase C deasserts MMIO:"
    echo "     OK=${FIX_CT_OK:-0} (non-MMIO deasserted), SKIP=${FIX_CT_SKIP:-0} (no working MMIO channels), FAIL=${FIX_CT_FAIL:-0}."
    if [ "${FIX_CT_SKIP:-0}" -gt 0 ]; then
        echo "     WARNING: ${FIX_CT_SKIP} skip(s) means ALL MMIO channels on relay device(s) were dead — no relay path"
        echo "     to deassert non-MMIO peers. Non-MMIO ERISCs stay in ROM boot → MMIO link training hangs at 0x49705180."
    fi
fi
if [ "${FIX_DC_PHASE_D:-0}" -gt 0 ]; then
    echo "  => [FIX DC] Phase D parallel poll (shared deadline): ${FIX_DC_PHASE_D:-0} channel(s) polled,"
    echo "     OK=${FIX_DC_PHASE_D_OK:-0}, FAIL=${FIX_DC_PHASE_D_FAIL:-0}."
    echo "     FIX DC replaces sequential 2s-per-channel polling with tight loop over all channels under one deadline."
    echo "     Expected speedup: 24ch × 2s = 48s → ~2s (single shared deadline)."
fi
if [ "${FIX_DC_BH:-0}" -gt 0 ]; then
    echo "  => [FIX DC] FIX BH parallel poll (Pass 2): ${FIX_DC_BH:-0} channel(s) polled,"
    echo "     OK=${FIX_DC_BH_OK:-0}, FAIL=${FIX_DC_BH_FAIL:-0}."
fi
if [ "${FIX_DD_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX DD] Transient PCIe read error in FIX BH parallel poll: ${FIX_DD_FIRES:-0} catch(es)."
    echo "     FIX DD wraps cluster.read_core() in try/catch so transient errors don't abort the poll loop."
fi
...
if [ "${FIX_BC_P5B_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX BC Phase 5b] simultaneous-handshake deadlock detected (${FIX_BC_P5B_FIRES} occurrence(s))."
    echo "     All stuck channels at REMOTE_HANDSHAKE_COMPLETE with peer chip IN quiescing_device_ids_."
    echo "     Both sides launched EDM simultaneously and are waiting for each other."
    echo "     Action: fabric_channels_not_ready_for_traffic_=true → FIX AA GTEST_SKIP."
    echo "     Log pattern: 'FIX BC (#42429): simultaneous-handshake deadlock detected'"
fi
if [ "${FIX_BD_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX BD AUDIT] Enhanced FIX BC logging with full channel-peer resolution (${FIX_BD_FIRES} occurrence(s))."
fi
...
echo "  FIX_BN_ROM_HANG:           ${FIX_BN_ROM_HANG:-0}  ([REGRESSION if > 0] 'did not exit ROM' — fw_launch_addr=0 race returned)"
echo "  FIX_BN_FW_LAUNCH_CLEAR:   ${FIX_BN_FW_LAUNCH_CLEAR:-0}  (fw_launch_addr cleared post-heartbeat or post-TERMINATE — FIX BN/PE path)"
echo "  FIX_BR_REGRESSION:         ${FIX_BR_REGRESSION:-0}  ([INFO] FIX PG/AC heartbeat timeout indicators — FIX BR gap if correlated with fw_launch_addr)"
echo "  FIX_BS_HEARTBEAT_STUCK:    ${FIX_BS_HEARTBEAT_STUCK_ABCD:-0}  ([REGRESSION if > 0] ETH heartbeat stuck at 0xabcd — FIX BS missing in UMD)"
echo "  FIX_BZ_FIRES:              ${FIX_BZ_FIRES:-0}  (fw_launch_addr_value written instead of 0 in FIX PE/BN paths — ERISC stays bootable)"
echo "  FIX_CA_FIRES:              ${FIX_CA_FIRES:-0}  (write-only reset of non-MMIO ETH ERISCs BEFORE FIX AC MMIO reset)"
echo "  FIX_CA_SUCCESS:            ${FIX_CA_SUCCESS:-0}  (FIX CA — all non-MMIO ERISCs reset successfully)"
echo "  FIX_CA_PARTIAL:            ${FIX_CA_PARTIAL:-0}  ([WARNING] FIX CA partial — some non-MMIO resets failed)"
echo "  FIX_CA_CORE_FAIL:          ${FIX_CA_CORE_FAIL:-0}  (FIX CA per-core failures — relay write-only threw)"
echo "  FIX_CB_FIRES:              ${FIX_CB_FIRES:-0}  (50ms delay after FIX CA — non-MMIO ERISCs given time to ROM boot)"
echo "  FIX_CC_FAIL:               ${FIX_CC_FAIL:-0}  ([WARNING if > 0] fw_launch_addr_value relay write to non-MMIO failed — link-training wait fallback)"
echo "  FIX_CD_FIRES:              ${FIX_CD_FIRES:-0}  ([WARNING if > 0] FIX PD/CD fw_launch_addr write failures in quiesce path)"
echo "  FIX_CE_SUCCESS:            ${FIX_CE_SUCCESS:-0}  (fw_launch_addr pre-write in teardown force-reset path)"
echo "  FIX_CE_FAIL:               ${FIX_CE_FAIL:-0}  ([WARNING if > 0] fw_launch_addr pre-write FAILED in teardown force-reset)"
echo "  FIX_CF_SUCCESS:            ${FIX_CF_SUCCESS:-0}  (fw_launch_addr pre-write in FIX RR-NM step 2 / FIX AY paths)"
echo "  FIX_CF_FAIL:               ${FIX_CF_FAIL:-0}  ([WARNING if > 0] fw_launch_addr pre-write FAILED in FIX RR-NM / FIX AY paths)"
echo "  FIX_CG_DUAL_FAIL:          ${FIX_CG_DUAL_FAIL:-0}  ([WARNING if > 0] non-MMIO channels unreachable via relay in BOTH FIX CA + FIX AY passes — FIX RR-NM needed)"
echo "  FIX_CH_FLUSH_TIMEOUT:      ${FIX_CH_FLUSH_TIMEOUT:-0}  ([WARNING if > 0] MMIO ERISC command-queue flush timeout during relay write)"
echo "  FIX_CI_ZERO_WRITE:         ${FIX_CI_ZERO_WRITE:-0}  (dispatch fw_launch_addr {0} write breadcrumbs — only > 0 with DEBUG logging enabled)"
echo "  FIX_CJ_BYPASS:             ${FIX_CJ_BYPASS:-0}  (bc_deadlock channels bypassed TERMINATE poll — immediate force-reset)"
echo "  FIX_CL_COLLECTIVE:         ${FIX_CL_COLLECTIVE:-0}  (collective assert-then-deassert events — simultaneous ETH link training)"
echo "  FIX_CL_DEASSERT_FAIL:      ${FIX_CL_DEASSERT_FAIL:-0}  ([WARNING if > 0] deassert failures in collective Phase 2)"
echo "  FIX_CN_SKIP:               ${FIX_CN_SKIP:-0}  (bc_deadlock channels skipped from fabric_firmware_init force-reset — delegated to FIX AC/AY)"
echo "  FIX_CO_SKIP:               ${FIX_CO_SKIP:-0}  (MMIO channels skipped in FIX AC — bc_deadlock non-MMIO peer unreachable)"
echo "  FIX_CP_SKIP:               ${FIX_CP_SKIP:-0}  (MMIO channels added to FIX AC skip from FIX CA-failed non-MMIO devices)"
...
echo "  FIX_BE_STALE_CLEAR:        ${FIX_BE_STALE_CLEAR:-0}  (stale per-cycle state cleared at configure entry)"
...
echo "  FIX_BY_BENIGN:             ${FIX_BY_BENIGN:-0}  (pre-init scan: booting/base-firmware channels — is_benign_erisc_state() fired)"
echo "  FIX_XX_FIRES:              ${FIX_XX_FIRES:-0}  (deassert guard — prevented deassert on non-asserted channel)"
echo "  FIX_FX_FIRES:              ${FIX_FX_FIRES:-0}  (persistent fixm_init flag used for Phase 5/5b timeout extension)"
echo "  FIX_QH1_FIRES:             ${FIX_QH1_FIRES:-0}  (P25-CLEAN channels excluded from Phase 5b health check)"
echo "  FIX_QH2_FIRES:             ${FIX_QH2_FIRES:-0}  (Phase 5b timeout extended for stale base-UMD channels)"
echo "  FIX_FQ1_FIRES:             ${FIX_FQ1_FIRES:-0}  (dispatch relay target validation — FIX FQ-1)"
echo "  FIX_FQ2_FIRES:             ${FIX_FQ2_FIRES:-0}  (post-init ETH FW verification — FIX FQ-2)"
echo "  FIX_FQ5_FIRES:             ${FIX_FQ5_FIRES:-0}  (dispatch enhanced diagnostics — FIX FQ-5)"
echo "  FIX_P25CV2_FIRES:          ${FIX_P25CV2_FIRES:-0}  (P25-CLEAN-V2: peer-in-quiesce-set gate)"
echo "  FIX_BW_FIRES:              ${FIX_BW_FIRES:-0}  (write_core_immediate relay failure → marks relay broken)"
echo "  FIX_BX_FIRES:              ${FIX_BX_FIRES:-0}  (initialize_and_launch_firmware exception guard)"
...
echo "  FIX_PE_FIRES:              ${FIX_PE_FIRES:-0}  (fw_launch_addr cleared for cleanly-terminated channels)"
echo "  FIX_QD_FIRES:              ${FIX_QD_FIRES:-0}  (health check skipped for dead non-MMIO — channels_not_ready set)"
echo "  FIX_RR_NM_FIRES:           ${FIX_RR_NM_FIRES:-0}  (relay-assisted ERISC reset for non-MMIO dead channels via MMIO PCIe)"
echo "  FIX_TB_FIRES:              ${FIX_TB_FIRES:-0}  (ASIC ID missing from topology mapping during re-discovery)"
echo "  FIX_BQ_FIRES:              ${FIX_BQ_FIRES:-0}  (typed catch for fabric_context already torn down)"
echo "  FIX_BU_FIRES:              ${FIX_BU_FIRES:-0}  (REMOTE_HANDSHAKE_COMPLETE early-exit deferred to Phase 5b)"
echo "  FIX_BT_FIRES:              ${FIX_BT_FIRES:-0}  (ROM postcode promoted to probe_dead_channels immediately)"
echo "  FIX_EXT2_FIRES:            ${FIX_EXT2_FIRES:-0}  (corrected external channel detection via get_ethernet_connections)"
echo "  FIX_RZ3_FIRES:             ${FIX_RZ3_FIRES:-0}  (persistent stale-base-UMD guard persists past ring-sync)"
echo "  FIX_RZ4_FIRES:             ${FIX_RZ4_FIRES:-0}  (transient channels_not_ready cleared/set around Phase 5b)"
...
if [ "${FIX_FX_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX FX] Persistent fixm_init flag used for Phase 5/5b timeout extension (${FIX_FX_FIRES} occurrence(s))."
    echo "     FIX RZ2 clears fabric_stale_base_umd_channels_ after ring-sync, but FIX FX uses"
    echo "     the persistent fabric_base_umd_fixm_init_ flag to keep extended timeouts active."
    echo "     Without FIX FX: Phase 5/5b timeouts revert to defaults → relay_path_broken on slow channels."
fi
if [ "${FIX_QH1_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX QH-1] P25-CLEAN channels excluded from Phase 5b health check (${FIX_QH1_FIRES} occurrence(s))."
    echo "     Channels already TERMINATED by FIX P25-CLEAN are not launched in Phase 3,"
    echo "     so Phase 5b must exclude them — otherwise they appear stuck at wrong status."
fi
if [ "${FIX_QH2_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX QH-2] Phase 5b timeout extended for stale base-UMD channels (${FIX_QH2_FIRES} occurrence(s))."
    echo "     kHealthCheckTimeoutMs increased from 2000ms to 24000ms when stale base-UMD detected."
fi
FIX_FQ_TOTAL=$(( ${FIX_FQ1_FIRES:-0} + ${FIX_FQ2_FIRES:-0} + ${FIX_FQ5_FIRES:-0} ))
if [ "$FIX_FQ_TOTAL" -gt 0 ]; then
    echo "  => [FIX FQ] Dispatch hang remediations: FQ-1=${FIX_FQ1_FIRES:-0} (relay target validation),"
    echo "     FQ-2=${FIX_FQ2_FIRES:-0} (post-init ETH FW verification), FQ-5=${FIX_FQ5_FIRES:-0} (enhanced diagnostics)."
fi
if [ "${FIX_P25CV2_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX P25-CLEAN-V2] Peer-in-quiesce-set gate fired (${FIX_P25CV2_FIRES} occurrence(s))."
    echo "     P25-CLEAN skip now conditional on peer NOT being in the quiesce set."
    echo "     Without V2: P25-CLEAN incorrectly skipped channels whose peer was in quiesce set."
fi
if [ "${FIX_BW_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX BW] write_core_immediate threw — relay marked broken (${FIX_BW_FIRES} occurrence(s))."
fi
if [ "${FIX_BX_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX BX] initialize_and_launch_firmware exception caught (${FIX_BX_FIRES} occurrence(s))."
    echo "     Non-MMIO device launch failed gracefully instead of crashing."
fi
...
if [ "${FIX_BG_SECOND_PASS_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX BG / audit #7] teardown second-pass partial failure(s): ${FIX_BG_SECOND_PASS_FIRES} event(s)."
    if [ "${FIX_BG_RELAY_DEAD_SKIP:-0}" -gt 0 ]; then
        echo "     - ${FIX_BG_RELAY_DEAD_SKIP} non-mmio channel(s): diagnostic read skipped (relay already confirmed dead)."
        echo "       Status logged as 0xdeaddead (sentinel). Channel registered for force-reset via FIX AC."
    fi
    if [ "${FIX_BG_ASSERT_SKIP:-0}" -gt 0 ]; then
        echo "     - ${FIX_BG_ASSERT_SKIP} channel(s): assert_risc_reset_at_core skipped (relay dead or prior assert failed)."
        echo "       Hardware reset delegated to RiscFirmwareInitializer::teardown (FIX AC PCIe-reset)."
    fi
    if [ "${FIX_BG_HB_TIMEOUT:-0}" -gt 0 ]; then
        echo "     - ${FIX_BG_HB_TIMEOUT} mmio channel(s): base firmware heartbeat not seen within 3000ms."
        echo "       Next session may see probe_dead on these channels. grep 'teardown second-pass:' for details."
    fi
fi
if [ "${FIX_NZ_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX NZ] Skipped initialize_and_launch_firmware for ${FIX_NZ_FIRES} non-MMIO device(s) with broken relay."
    echo "     Without FIX NZ: 64-core Tensix init × 5s UMD timeout = 320s hang before GHA kills the job."
fi
if [ "${FIX_RR_NM_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX RR-NM] Relay-assisted ERISC reset for non-MMIO dead channels (${FIX_RR_NM_FIRES} occurrence(s))."
    echo "     Uses MMIO device's PCIe to soft-reset ERISC on the non-MMIO side of a dead link."
fi
if [ "${FIX_BT_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX BT] ROM postcode channel(s) promoted to probe_dead immediately (${FIX_BT_FIRES} occurrence(s))."
    echo "     Without FIX BT: 5s batch poll wasted — ROM-postcode ERISC cannot answer command-queue probes."
fi
if [ "${FIX_AY_C_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX AY-C] Per-pair MMIO relay readiness guard in teardown (${FIX_AY_C_FIRES} occurrence(s))."
    echo "     Prevents force-reset on channels whose MMIO peer is not yet ready — avoids relay deadlock."
fi
if [ "${FIX_AQ2_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX AQ-2] UMD topology discovery skipped unreachable device(s) (${FIX_AQ2_FIRES} occurrence(s))."
    echo "     Without FIX AQ-2: topology re-discovery hangs probing dead links."
fi
...
if [ "${FIX_E2_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX E2] Dead-relay device skipped configure_fabric + dispatch kernel init ${FIX_E2_FIRES} time(s)."
fi
if [ "${FIX_G_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX G] Dead-relay device skipped router sync / health verification ${FIX_G_FIRES} time(s)."
fi
if [ "${FIX_H_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX H] MMIO device had all ETH channels dead after configure_fabric_cores ${FIX_H_FIRES} time(s)."
fi
if [ "${FIX_I2_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX I2] MMIO master router connects to dead-relay peer (transitive skip) ${FIX_I2_FIRES} time(s)."
fi
if [ "${FIX_U_FIRES:-0}" -gt 0 ]; then
    echo "  => [FIX U] Non-MMIO master channels not ready after Phase 5 poll ${FIX_U_FIRES} time(s)."
fi
