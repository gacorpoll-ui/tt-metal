...
def _peel_srcab_only(fmt: DataFormat) -> DataFormat:
    """
    ...
    """
    if fmt == DataFormat.MxFp4_2x_A:
        return DataFormat.Float16
    if fmt == DataFormat.MxFp4_2x_B:
...
def infer_data_formats(
    ...
    # 2x register-only formats cannot appear as math/pack format. Peel them back to
    # the paired non-2x family member for downstream inference; unpack_*_dst still
    # carries the actual 2x format below.
    unpack_out_A_eff = _peel_srcab_only(unpack_out_A)
    unpack_out_B_eff = _peel_srcab_only(unpack_out_B)
    ...
    math = infer_math_format(unpack_out_A_eff, unpack_out_B_eff)
