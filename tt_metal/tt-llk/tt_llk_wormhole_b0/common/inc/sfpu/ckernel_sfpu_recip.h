...
#pragma GCC unroll 8
    for (int d = 0; d < iterations; d++)
    {
        ...
        {
            sfpi::dst_reg[(dst_index_out - dst_index_in) * 32] = _sfpu_reciprocal_<0>(in);
        }
        else
        {
            if constexpr (is_fp32_dest_acc_en)
            {
                sfpi::dst_reg[(dst_index_out - dst_index_in) * 32] = _sfpu_reciprocal_<2>(in);
            }
            else
            {
                sfpi::vFloat out = _sfpu_reciprocal_<1>(in);
                sfpi::dst_reg[(dst_index_out - dst_index_in) * 32] = sfpi::float_to_fp16b(out, sfpi::RoundMode::NearestEven);
            }
        }
