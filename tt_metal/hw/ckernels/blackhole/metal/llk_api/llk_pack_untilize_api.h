...
template <
    ...
    bool dense = false>
inline void llk_pack_untilize_init(std::uint32_t output, std::uint32_t face_r_dim = 0, std::uint32_t num_faces = 0) {
    static_assert(diagonal == false, "Diagonal is only supported on WH");
    const std::uint32_t output_id = get_output_id(output);
    face_r_dim = face_r_dim == 0 ? get_output_face_r_dim(output_id) : face_r_dim;
    num_faces = num_faces == 0 ? get_output_num_faces(output_id) : num_faces;
