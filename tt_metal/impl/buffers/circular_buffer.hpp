...
public:
    ...
    // Write computed scratch address back into the originating CBDescriptor so Python callers
    // (e.g. blaze.l1_profile.print_cb_stats) can read it via ttnn.get_cb_address().
    void write_back_allocated_address(uint32_t address) {
        if (source_descriptor_) {
            source_descriptor_->address_offset = address;
        }
    }
...
private:
    ...
    // Non-owning back-pointer to the originating CBDescriptor (set only for scratch CBs built
    // from a descriptor, nullptr for programmatic CBs).  Lifetime: valid as long as the
    // ProgramDescriptor / MeshProgramDescriptor that owns the descriptor is alive.
    CBDescriptor* source_descriptor_ = nullptr;
